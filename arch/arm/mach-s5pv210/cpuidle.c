...
static int check_idmapos(void)
{
        ...
        src = src & 0x3FFF;
        src = 0x4000 - src;
        if(src < 0x150){             // 0x150? PCM dataêž°ì??Œë¡ ??2ms
                return 1;
        }
        else
                return 0;
...
static int check_rtcint(void)
{
        ...
        if(current_cnt < 0x40){       // 0x40? RTC clock??32768êž°ì??Œë¡ 2ms??
                return 1;
        }
        else
                return 0;
...
//      tmp &= ~((1<<2) | (1<<5) | (1<<13));
	...
	/* Entering didle mode with WFI instruction */
		s5p_didle();
...
static spinlock_t didle_lock;
int didle_lock_count = 0;
/*
 * flag = 0 : allow to enter lowpower (idle, didle) mode.
 * flag = 1 : not allow to enter lowpower (idle, didle) mode.
 */
void s5p_set_lowpower_lock(int flag)
{
	spin_lock(&didle_lock);
	...
	spin_unlock(&didle_lock);
}
EXPORT_SYMBOL(s5p_set_lowpower_lock);

int s5p_get_lowpower_lock(void)
{
	return didle_lock_count;
}
EXPORT_SYMBOL(s5p_get_lowpower_lock);
...
/* Actual code that puts the SoC in different idle states */
static int s5p_enter_idle_lowpower(struct cpuidle_device *dev,
				struct cpuidle_state *state)
{
	struct timeval before, after;
	int idle_time;
				...
				struct cpuidle_state *state)
{
	if (s5p_idle_bm_check())
		...
		return s5p_enter_idle_lowpower(dev, state);
}
...
int s5p_setup_lowpower(unsigned int mode)
{
	...
	case LOWPOWER_MODE:
		...
		device->states[0].flags = CPUIDLE_FLAG_TIME_VALID;
		strcpy(device->states[0].name, "DIDLE");
		strcpy(device->states[0].desc, "S5PV210 didle");
...
EXPORT_SYMBOL(s5p_setup_lowpower);
